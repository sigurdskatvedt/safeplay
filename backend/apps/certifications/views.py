...
class CertificationRequestViewSet(viewsets.ModelViewSet):
    ...
    # override delete method to check if certification request is pending
    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        if instance.status != Status.PENDING or instance.user != self.request.user:
            return Response({'error': 'You can only delete your own certification requests if the status is pending'}, status=status.HTTP_400_BAD_REQUEST)
